...
# or
element_screenshot ${elementId} ${path}
```
Default the screenshot saved to `./screenshot.png`.
...
element_screenshot $logo ./logo.png
```

## 11.2 Get Page Source
...
Shellnium provides convenience wrappers for [W3C WebDriver Actions](https://www.w3.org/TR/webdriver/#actions).
...
# Selects a word by double-clicking
```
...
# Ctrl+V (paste)
send_key_combo "$KEY_CONTROL" "v"
```

### Low-Level Actions API

## 12.10 Perform Actions
Execute low-level [W3C WebDriver Actions](https://www.w3.org/TR/webdriver/#perform-actions). Accepts a JSON string describing the actions to perform.

### Command
```bash
perform_actions ${actions_json}
```
...
release_actions
# All keys and buttons are now released
```
